...
class TaxaImages(Base):
    ...
    def __repr__(self):
       return "<TaxaImages(taxonName='%s', subfamily='%s', genus='%s', species='%ds', subspecies='%s', code='%s' \
...
def getTaxaImages():
    ...
    uploadDate = request.args.get('upload_date', default='*', type=str)
    ...
    query = session.query(TaxaImages)
    if (taxonName != '*'):
      query = query.filter(TaxaImages.taxonName == taxonName)
    if (subfamily != '*'):
      query = query.filter(TaxaImages.subfamily == subfamily)
    if (genus != '*'):
      query = query.filter(TaxaImages.genus == genus)
    if (species != '*'):
      query = query.filter(TaxaImages.species == species)
    if (subspecies != '*'):
      query = query.filter(TaxaImages.subspecies == subspecies)
    if (code != '*'):
      query = query.filter(TaxaImages.code == code)
    if (uid != '*'):
      query = query.filter(TaxaImages.uid == uid)
    if (uploadDate != '*'):
      query = query.filter(TaxaImages.uploadDate == uploadDate)
    if (shotType != '*'):
      query = query.filter(TaxaImages.shotType == shotType)
    query.order_by(TaxaImages.taxonName, TaxaImages.code) 
    ...
    #imageCoun t = getImageCount(taxaDict)
...
def getTaxaDict(taxaImage, taxaDict):
    ...
    # Create a new taxonDict, in case it doesn't exist yet.
    ...
    #print('getTaxaDict() taxonName:' + taxaImage.taxonName)
    specimensDict = taxonDict['specimens']    

    if taxaImage.code not in specimensDict:
      specimenDict = {
        'code': taxaImage.code
      , 'images': []
      }
      specimensDict[taxaImage.code] = specimenDict
      #print('getTaxaDict() code:' + taxaImage.code)
    else:
      specimenDict = specimensDict[taxaImage.code]
    #print('getTaxaDict() code:' + str(len(specimenDict)) + ' code:' + taxaImage.code)
...
def getTaxonCount(taxaDict):
    print("taxon Count:" + str(len(taxaDict)))    
    return len(taxaDict)
...
def getSpecimenCount(taxaDict):
    ...
    for taxonDict in taxaDict:
      ...
      #print("specimen Count:" + str(len(taxonDict['specimens'])) + " str:" + str(taxonDict) +  ' val:' + val)    
      #print('val:' + str(val))
      #print("specimenCount() len:" + str(len(val['specimens'])))
      specimenCount = specimenCount + len(val['specimens'])
...
def getImageCount(taxaDict):
    ...
    for key in taxaDict:
      ...
      #print("specimen Count:" + str(len(taxonDict['specimens'])) + " str:" + str(taxonDict) +  ' taxonDict:' + taxonDict)    
      #print('taxonDict:' + str(taxonDict))
      print(" len:" + str(len(taxonDict['specimens'])))

      specimensDict = taxonDict['specimens']
      for key in specimensDict:
        specimenDict = specimensDict[key]
...
def getSpecimen():
    ...
    if (maxElevation != '*') :
      query = query.filter(Specimens.minimumElevationInMeters < maxElevation)   
    if (museum != '*') :
      query = query.filter(Specimens.museum == museum)
    if (ownedby != '*') :
      query = query.filter(Specimens.ownedby == ownedby)
    if (locatedat != '*') :
      query = query.filter(Specimens.locatedat == locatedat)
    if (collectedby != '*') :
      query = query.filter(Specimens.collectedby == collectedby)
