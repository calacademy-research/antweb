<%@ page errorPage = "/error.jsp" %><%@ taglib uri="/WEB-INF/struts-bean.tld" prefix="bean" %><%@ taglib uri="/WEB-INF/struts-tiles.tld" prefix="tiles" %><%@ page import="org.calacademy.antweb.Group" %><%@ page import="org.calacademy.antweb.Utility" %><%@ page import="org.calacademy.antweb.AncFile" %><%@ page import="org.calacademy.antweb.util.*;" %><%    if (!org.calacademy.antweb.util.HttpUtil.isInWhiteListCheck(request.getQueryString(), response)) return;     String domainApp = (new Utility()).getDomainApp(); %><%@include file="/common/antweb-defs.jsp" %><tiles:insert beanName="antweb.default" beanScope="request" flush="true">	<tiles:put name="title" value="AntWeb Participation" />	<tiles:put name="body-content" type="string"><div id="page_contents">	   <h1>AntWeb Participation</h1><div class="clear"></div><div class="page_divider"></div></div><div id="page_data"><div id="overview_data">	   	   <style type="text/css">
...
<ul>
	...
	<li>
		List of Countries and adm1s for each <a href="https://www.antweb.org/bioregionCountryList.do" target="_blank">Bioregion</a></li>
	<li>
		<a href="https://www.antweb.org/countryAdm1List.do" target="_blank">List of Country</a> names and alternate names</li>
</ul>
<h3>
...
<ul>
	<li>
	   ...
	   <%        AncFile ancFile = (AncFile) session.getAttribute("ancFile");	           Group accessGroup = (Group) request.getSession().getAttribute("accessGroup");        if (accessGroup != null) {          String requestURL = request.getRequestURL().toString();          String accessIdStr = "/" + (new Integer(accessGroup.getLogin().getId())).toString() + "/";	      if ( (accessGroup.isAdmin())	        || (accessGroup.getProjectNames().contains("homepage"))   	        || (requestURL.contains(accessIdStr))     	    || (requestURL.contains("curators"))	            	         ) {	   %>	   <form method="POST" action="<%= domainApp %>/ancPageEdit.do?id=10" />	   		<input type="submit" value="Edit Page">	   	</form>	   	 <% if (!(session.getAttribute("ancFile") == null)) { %>	   	<form method="POST" action="<%= domainApp %>/ancPageSave.do"> 	   		<input type="submit" value="Save Page">	   	</form>         <% } %>       <% } %>	   		 <% } %>	   		   	</div>	   	</div>	</tiles:put></tiles:insert>