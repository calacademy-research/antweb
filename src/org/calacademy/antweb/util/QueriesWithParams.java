...
public abstract class QueriesWithParams {
    ...
    public static NamedQuery getNamedQueryWithParam(String queryName, String geolocale) {

        if ("speciesListWithRangeData".equals(queryName)) {
            return getSpeciesListWithRangeData(geolocale);
        }
        return null;
    }
    ...
    private static NamedQuery getSpeciesListWithRangeData(String geolocaleName) {
        ...
        query = new NamedQuery(
                ...
                , "select t.subfamily, t.genus, t.species, IFNULL(t.subspecies, ''), t.author_date, t.status, gt.is_introduced, gt.is_endemic, "
                  + " (select group_concat(g2.name) from geolocale_taxon gt2, geolocale g2 where gt2.geolocale_id = g2.id and g2.georank = '" + georank + "' and gt2.taxon_name = t.taxon_name order by g2.name) "
