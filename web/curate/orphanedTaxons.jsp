...
<hr></hr>
Listed here are taxons that are believed to be "orphans".  In this case that means that
...
<% String lastSource = source;
   ...
   <hr>
   <h3>Source: <%= source %></h3>
     Delete all taxons (listed below) for this source (that don't have specimens and don't have description edits): <a href="<%= domainApp %>/orphanedTaxons.do?source=<%= source %>&action=delete"><img border=0" src="<%= domainApp %>/image/delete.png"></a>
   <hr>
<% } %>

<% int orphan_index = orphanCount.intValue() + 1; %>
<bean:write name="orphan" property="created"/>
<a href="<%= domainApp %>/orphanedTaxons.do?taxonName=<%= ((Taxon) orphan).getTaxonName() %>&action=delete"><img border=0" src="<%= domainApp %>/image/delete.png"></a>
