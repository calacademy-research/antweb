...
<% // This warning is to prevent developers (me) from accidentally posting data to the live site.
   ...
   //A.log("curate-bodyB.jsp hasServerMessage:" + AntwebMgr.hasServerMessage() + " : " + AntwebMgr.getServerMessage());
        ...
        %>
                ...
                for (String uploadFileKind : uploadFileKindList) {
                  if (LoginMgr.isAdmin(accessLogin)) { 
                    if (uploadFileKind.contains("specimen")) {
                      //if (AntwebProps.isDevMode()) AntwebUtil.log("curate-bodyB.jsp kind:" + uploadFileKind); %>
            ...
            <%
                    ...
                    if (specIndex > 8) {
                      ...
                      if (AntwebProps.isDevMode()) AntwebUtil.log("curate-bodyB.jsp fileAccessId:" + fileAccessId);
...
-->
 ...
 <% } 
  ...
  <%  } 
      ...
      AntwebUtil.log("curate-bodyB.jsp projList is null for:" + HttpUtil.getTarget(request));
    }
  %>
...
<% 
if (AntwebProps.isDevOrStageMode()) {
  //AntwebUtil.log("curate-bodyB.jsp speciesList:" + speciesList);
}
...
<html:form method="POST" action="upload.do" enctype="multipart/form-data">
  ...
  <html:select property="editSpeciesList">
	...
	<% for (SpeciesListable s : groupSpeciesList) { 
         ...
         //A.log("curate-bodyB.jsp key:" + s.getKey() + " title:" + s.getTitle());
              ...
              <select name="viewpages_values">
                ...
                <% if (projList != null) {
                     for (SpeciesListable s : projList) { 
                       ...
                       if (p == null) {
                         //A.log("curate-bodyB.jsp 1 p is null for:" + s.getName());
                       } else { %>
...
< % } % >
	...
	<% if (projList != null) {
	     for (SpeciesListable s : projList) { 
	       Project p = ProjectMgr.getProject(s.getName());
	       if (p == null) {
	         //A.log("curate-bodyB.jsp 2 p is null for:" + s.getName());
	       } else { %>
