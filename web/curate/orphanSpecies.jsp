...
<hr></hr>
Listed here are species that are believed to be "orphans".  In this case that means that
it has no specimen records, and no description edits, and it's source is not a project list.  
...
<% String lastSource = source;
   ...
   <hr>
   <h3>Source: <%= source %></h3>
     Delete all species (listed below) for this source (that don't have specimens and don't have description edits): <a href="<%= domainApp %>/orphanSpecies.do?source=<%= source %>&action=delete"><img border=0" src="<%= domainApp %>/image/delete.png"></a>
   <hr>
<% } %>

<% int orphan_index = orphanCount.intValue() + 1; 
   String orphanTaxonName = ((Taxon)orphan).getTaxonName();
%>
<bean:write name="orphan" property="created"/>
<a href="<%= domainApp %>/orphanSpecies.do?taxonName=<%= ((Taxon) orphan).getTaxonName() %>&action=delete"><img border=0" src="<%= domainApp %>/image/delete.png"></a>
&nbsp;&nbsp;&nbsp; <bean:write name="orphan" property="rank"/>:
 <%= orphan_index %>. <a href="<bean:write name="orphan" property="fullUrl"/>"><%= ((Taxon) orphan).getLongPrettyTaxonName() %></a> 
<hr></hr>
</logic:iterate>
