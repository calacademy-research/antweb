...
<%
  AntwebUtil.log("curate-bodyC.jsp");

if (org.calacademy.antweb.util.HttpUtil.isStaticCallCheck(request, out)) {
  AntwebUtil.log("curate-bodyC.jsp static call:" + HttpUtil.getTarget(request));
  return;
}
%>
...
<% // This warning is to prevent developers (me) from accidentally posting data to the live site.
   ...
   //A.log("curate-bodyC.jsp hasServerMessage:" + AntwebMgr.hasServerMessage() + " : " + AntwebMgr.getServerMessage());
...
<br>If not returned an upload report, find it in the <a href='<%= domainApp %>/listSpecimenUploads.do?groupId=<%= accessGroup.getId() %>'>Specimen Upload Reports</a>.
     ...
     <% } %>
        ...
        <% 
            ...
            //if (AntwebProps.isDevMode()) AntwebUtil.log("curate-bodyC.jsp uploadFileKindList:" + uploadFileKindList);
        %>
                ...
                for (String uploadFileKind : uploadFileKindList) {
                  if (LoginMgr.isAdmin(accessLogin)) { 
                    if (uploadFileKind.contains("specimen")) {
                      //if (AntwebProps.isDevMode()) AntwebUtil.log("curate-bodyC.jsp kind:" + uploadFileKind); %>
            ...
            <%
                    ...
                    if (specIndex > 8) {
                      ...
                      if (AntwebProps.isDevMode()) AntwebUtil.log("curate-bodyC.jsp fileAccessId:" + fileAccessId);
...
-->
 ...
 <% } 
  ...
  <%  } 
      ...
      AntwebUtil.log("curate-bodyC.jsp projList is null for:" + HttpUtil.getTarget(request));
    }
  %>
...
<% 
if (AntwebProps.isDevOrStageMode()) {
  //AntwebUtil.log("curate-bodyC.jsp speciesList:" + speciesList);
}
...
<html:form method="POST" action="upload.do" enctype="multipart/form-data">
  ...
  <html:select property="editSpeciesList">
	...
	<% for (SpeciesListable s : groupSpeciesList) { 
         ...
         //A.log("curate-bodyC.jsp key:" + s.getKey() + " title:" + s.getTitle());
              ...
              <select name="viewpages_values">
                ...
                <% if (projList != null) {
                     for (SpeciesListable s : projList) { 
                       ...
                       if (p == null) {
                         //A.log("curate-bodyC.jsp 1 p is null for:" + s.getName());
                       } else { %>
...
< % } % >
	...
	<% if (projList != null) {
	     for (SpeciesListable s : projList) { 
	       Project p = ProjectMgr.getProject(s.getName());
	       if (p == null) {
	         //A.log("curate-bodyC.jsp 2 p is null for:" + s.getName());
	       } else { %>
...
<%
  AntwebUtil.log("curate-bodyC.jsp END");
%>
