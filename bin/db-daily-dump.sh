...
if [ -d $backupdir ]; then
  docker exec antweb_mysql_1 mysqldump -u $dbuser --password=$dbpass --routines  --skip-lock-tables --default-character-set=utf8 --databases "$dbname" | tail -n +2 | gzip -c -9 > "$tempfile"
